...
text <- text.prepare("en_US.twitter.txt", n.lines = 120000)
...
model %>%
    layer_lstm(128, input_shape = c(maxlen, length(chars)), kernel_regularizer = regularizer_l2(0.0001)) %>%
    layer_dropout(0.5) %>%
    layer_dense(length(chars), kernel_regularizer = regularizer_l2(0.0001)) %>%
